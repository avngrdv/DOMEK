...
class Template:
    ...
    def __call__(self, loc, return_mask=False):
        '''
        fancy indexing to get the sequence of regions 
        at a specified location 'loc'
        '''
    ...
    def rebuild(self):
        '''
        Rebuild the object from the internally stored information 
        (lib_seq, etc). Used to revert the design back to the 
        original when it's truncated using the 'truncate_and_reindex' op.
        '''
        return self._build()
