...
def from_matrix_v3(X, alphabet=None, F=None, reshape=True, repad=False):
 ...
 alphabet:     Current representations of X
   ...
   reshape:    Should be flagged if a peptide sequence is to be represented as
    ...
    #previous versions of this code attempted to infer the alphabet
    #by running np.unique on X. this can fail badly e.g., if not all 
    #possible tokens encoded in the actual alphabet are present in the
    #dataset. therefore, I decided to stop with the inference and now
    #demand supplying the alphabet explicitly.
    try:
        alphabet = np.asarray(alphabet)
    except:
        raise ValueError('<featurize.from_matrix_v3> received an invalid "alphabet" variable that could not be coerced into an array.')

    #special treatment for padded arrays: try to rescue featurization
    #the problem is that the feature matrix will probably not have a row
    #to encode the pads (should be all zeros most of the time)
    _is_padded = -1 in X
    if _is_padded:
...
X array have incompatible dimensions; cannot perform featurization. . .')
    ...
    #if F is None, ie., if we are doing one-hot, the rightmost column
    #needs to be gotten rid if the initial X is padded
    else:
        if _is_padded:
            fX = fX[:,:-1]
...
def RFA_featurization(X, alphabet=None, order=None):
    ...
    models. Refer to Nat Commun 2024, 15, 7953 for further details.
        ...
        Returns
        -------
           A flattened array (2D) holding featurized peptide representations 
           row-wise
    ...
    if order != 'first' and order != 'second':
        raise ValueError('<RFA_featurization>: the "order" keyword not understood: only "first" or "second" are allowed values!')
