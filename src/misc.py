...
def compute_sublib_proba(X, y, pos_ind, alphabet):
    '''
    Compute pairwise epi scores. See the manuscript for details.
    Computation is not optimized, so it may take a while for large X.
    ...
    n_dims = len(pos_ind)    
    proba = np.zeros((n_aas,) * n_dims, dtype=np.float32)
    std = np.zeros((n_aas,) * n_dims, dtype=np.float32)
    
    iterable = (np.arange(n_aas),) * n_dims
    
    #this is brilliant
    for ind in itertools.product(*iterable):
        
        
        mask = np.zeros((arr_size, len(ind)), dtype=np.bool)
        for i,aa in enumerate(ind):
            mask[:,i] = X[:,pos_ind[i]] == aa
        
        mask = mask.all(axis=1)
        proba[ind] = y[mask].mean()
        std[ind] = y[mask].std()
        print(f'{ind} proba computed. . .')
