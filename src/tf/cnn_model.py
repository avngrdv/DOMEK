...
def cnn_v7(inp_dim=None, drop=0.05):
...
def domek_summary(x, size=256, drop=0.05):
    ...
    x = K.layers.Dense(size, activation='relu', kernel_initializer='he_normal')(x)
    x = K.layers.Dropout(drop)(x)
    x = K.layers.Dense(1, activation=None, kernel_initializer='he_normal')(x)
    out = ReLU(max_value=6.514936545834624)(x)    
    return out

def cnn_vF(inp_dim=None, drop=0.05):
...
def cnn_vj(inp_dim=None, drop=0.05):
...
def cnn_vm_J(inp_dim=None, drop=0.05):
...
def cnn_vm_J3(inp_dim=None, drop=0.05):
...
def domek_cnn(inp_dim=None, drop=0.05):
